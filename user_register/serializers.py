...
class User_registerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User_register
        fields = ('pk','token_generated','company_photo','photo','firstname', 'lastname','title', 'email', 'phone','vz_id','industry','company','address_line_1','address_line_2','city','pin_code','otp_generated')
    ...
    def create(self, validated_data):
        ...
        if(str(validated_data.get('phone'))[:2] == '91'):
          ...
          r = send_message(sid, token,
              ...
              sms_body='Hi '+validated_data.get('phone')+', Your one time password for VzCards login is 444444. Please use the password to login to the app.')
          print r.status_code
          pprint(r.json())
          ##--------------------------------------------
          ...
          params = {
              ...
              'from': 'NEXMO',
              'text': 'Hi '+validated_data.get('phone')+', Your one time password for VzCards login is 444444. Please use the password to login to the app.'
          }
        ...
        if User_register.objects.filter(phone=validated_data.get('phone')).exists():
          User_register.objects.filter(phone=validated_data.get('phone')).update(otp_generated=444444)
        if User_register.objects.filter(phone=validated_data.get('phone')).exists():
